...
namespace BioNetDAL
{
    public class DataObjects
    {
        ...
        public PsReponse InsertMauSMS(PSDanhMucMauSMSC sms)
        {
            PsReponse res = new PsReponse();
            try
            {
                ...
                {
                    db.PSDanhMucMauSMSCs.InsertOnSubmit(sms);
                    db.SubmitChanges();
                }
                else
                {
                    var sm = db.PSDanhMucMauSMSCs.FirstOrDefault(x => x.RowIDMauSMS == sms.RowIDMauSMS);
        ...
        #region SMS
        public List<PSDanhMucMauSMSC> LoadMauSMS(string HinhThucGuiTN, string DoituongNhanTinNhan, string Noidunggui)
        {
            List<PSDanhMucMauSMSC> danhmucsms = new List<PSDanhMucMauSMSC>();
            try
            {
                danhmucsms = db.PSDanhMucMauSMSCs.OrderBy(y => y.STT).Where(x => x.HinhThucGuiTN == HinhThucGuiTN && x.DoiTuongNhanTN == DoituongNhanTinNhan && x.NoidungGui == Noidunggui).ToList();
            }
            catch
            {
            }
                    ...
                    foreach (var ph in phieu)
                    {
                        ...
                        if (pa != null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(pa.MotherPhoneNumber) && SDT1 == false)
                            {
                                ...
                                if (pa.MotherPhoneNumber.Trim().StartsWith("0") == true)
                                {
                                    dsms.SDTNguoiNhan = "84" + pa.MotherPhoneNumber.Trim().Substring(1);
                                }
                                else if (pa.MotherPhoneNumber.Trim().StartsWith("84") == true)
                                {
                                    dsms.SDTNguoiNhan = pa.MotherPhoneNumber.Trim();
                                }
                            }
                            else if (!string.IsNullOrEmpty(pa.FatherPhoneNumber) && SDT1 == true)
                            {
                                dsms.TenNguoiNhan = "anh " + pa.FatherName;
                                if (pa.FatherPhoneNumber.Trim().StartsWith("0") == true)
                                {
                                    dsms.SDTNguoiNhan = "84" + pa.FatherPhoneNumber.Trim().Substring(1);
                                }
                                else if (pa.FatherPhoneNumber.Trim().StartsWith("84") == true)
                                {
        ...
        public List<PSXN_KetQua> GetDSPhongXN(bool isDaCoKQ)
        {
            ...
            try
            {
                ...
                {
                    var results = db.PSXN_TraKetQuas.Where(p => p.isXoa != true && p.isDaDuyetKQ != true).OrderBy(p => p.MaPhieu).ToList();
                }
        ...
        public List<PSGanViTriXNKQ> GetDSChuaGanXN()
        {
            ...
            try
            {
                ...
                foreach (var c in kq)
                {
                    ...
                    var duyet = db.PSCM_GanViTris.FirstOrDefault(x => x.MaPhieu.Equals(gan.MaPhieu) && x.MaGoiXN.Equals(gan.MaGoiXN) && x.MaXetNghiem.Equals(gan.MaXetNghiem) && x.isXoa != true);
                    if (duyet != null)
                    {
        ...
        public string GetMaRowIDLanDucLo(string MayDL)
        {
            string Dau = "GVT" + DateTime.Now.Year + DateTime.Now.Month + DateTime.Now.Day;
            string ma = string.Empty;
            try
            {
                var res = db.PSCM_DSGanMays.FirstOrDefault(x => x.isDaDuyet != true && x.isDaXoa != true && x.IDMayDucLo.Equals(MayDL));
                if (res != null)
                {
                    if(string.IsNullOrEmpty(res.IDLanDucLo))
                    {
                        res.IDLanDucLo= "GVT" + res.IDRowLanDucLo;
                        db.SubmitChanges();
                    }
                    ma = res.IDLanDucLo;
                }
                ...
                {
                    PSCM_DSGanMay may = new PSCM_DSGanMay();
                    may.IDMayDucLo = MayDL;
                    may.DateStart = DateTime.Now;
                    may.IDNVStart = "";
                    may.isDaXoa = false;
                    may.isDaDuyet = false;
                    db.PSCM_DSGanMays.InsertOnSubmit(may);
                    db.SubmitChanges();
                    ma = GetMaRowIDLanDucLo(MayDL);
                }
            }
            catch
            {
        ...
        public PsReponse SaveGanXN(PSCMGanViTriChung pm)
        {
            ...
            db.SubmitChanges();
            //PSCM_GanViTri viTri = db.PSCM_GanViTris.FirstOrDefault(x => x.MaPhieu == pm.MaPhieu && x.MaXetNghiem == pm.MaXetNghiem && x.IDLanDucLo==pm.IDLanDucLo
            //&& x.STT_bang == pm.STT_bang && x.MaGoiXN == pm.MaGoiXN && x.isDaDuyet != true);
            PSCM_GanViTri viTri = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(pm.IDLanDucLo)
            ...
            if (viTri != null)
            {
                if (pm.MAYXN001 != null)
                {
                    ...
                    ganct.IDLanDucLo = viTri.IDLanDucLo;
                    ganct.IDRowGanXN = viTri.IDRowGanXN;
                    ganct.isDaDuyet = false;
                    ganct.IDMayXN = "MAYXN001";
                    ganct.STT = pm.MAYXN001.STT;
                    ganct.STTDia = pm.MAYXN001.STTDia;
                    ganct.STTVT = pm.MAYXN001.STTVT;
                    ganct.isTest = pm.MAYXN001.isTest;
                    ganct.GhiChuCT = pm.MAYXN001.GhiChuCT;
                    ganct.ViTri = pm.MAYXN001.ViTri;
                    gan.isXoa = false;
                ...
                if (pm.MAYXN002 != null)
                {
                    ...
                    ganct.IDRowGanXN = viTri.IDRowGanXN;
                    ganct.isDaDuyet = false;
                    ganct.IDMayXN = "MAYXN002";
                    ganct.STT = pm.MAYXN002.STT;
                    ganct.STTDia = pm.MAYXN002.STTDia;
                    ganct.STTVT = pm.MAYXN002.STTVT;
                    ganct.isTest = pm.MAYXN002.isTest;
                    ganct.GhiChuCT = pm.MAYXN002.GhiChuCT;
                    ganct.ViTri = pm.MAYXN002.ViTri;
                    gan.isXoa = false;
                    db.PSCM_GanViTriCTs.InsertOnSubmit(ganct);
                    db.SubmitChanges();
                }
        ...
        public PsReponse SuaDanhSachGanXNLuu(PSCMGanViTriChung pm,string STT,string IDLanDucLo)
        {
            PsReponse gvc = new PsReponse();
            try
            {
                PSCM_GanViTri viTri = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(IDLanDucLo) && x.STT_bang == long.Parse(STT) && x.isDaDuyet != true);
                if (viTri != null)
                {
        ...
        public PsReponse SuaDanhSachGanXNLuuNew(PSCMGanViTriChung pm, string MayDucLo, string IDLanDucLo, string MaNV)
        {
            PsReponse gvc = new PsReponse();
            try
            {
                var dSGanMay = db.PSCM_DSGanMays.FirstOrDefault(x => x.IDMayDucLo.Equals(MayDucLo) && x.isDaDuyet != true && x.isDaXoa != true && x.IDLanDucLo.Equals(IDLanDucLo));
                if (dSGanMay == null)
                {
                    PSCM_DSGanMay ganMay = new PSCM_DSGanMay();
                    ganMay.IDLanDucLo = IDLanDucLo;
                    ganMay.IDMayDucLo = MayDucLo;
                    ganMay.IDNVStart = MaNV;
                    ganMay.DateStart = DateTime.Now;
                    ganMay.isDaDuyet = false;
                    ganMay.isDaXoa = false;
                    db.PSCM_DSGanMays.InsertOnSubmit(ganMay);
                    db.SubmitChanges();
                }
                else
                {
                    if(string.IsNullOrEmpty(dSGanMay.IDNVStart))
                    {
                        dSGanMay.IDNVStart = MaNV;
                        dSGanMay.DateStart = DateTime.Now;
                        db.SubmitChanges();
                    }
                }
                int STTbang = 0;
                PSCM_GanViTri GanVTNew = new PSCM_GanViTri();
                GanVTNew.IDMayDucLo = MayDucLo;
                GanVTNew.MaGoiXN = pm.MaGoiXN;
                GanVTNew.IDLanDucLo = IDLanDucLo;
                GanVTNew.MaXetNghiem = pm.MaXetNghiem;
                GanVTNew.MaPhieu = pm.MaPhieu;
                GanVTNew.isDaDuyet = false;
                GanVTNew.isXoa = false;
                #region GanVTTest
                foreach (var pmMay in pm.may)
                {
                    ...
                    do
                    {
                        ...
                        var listGanMay = db.PSCM_GanViTriCTs.Where(x => x.isDaDuyet != true && x.isXoa != true && x.IDLanDucLo.Equals(IDLanDucLo) && x.IDMayXN.Equals(pmMay.IDMayXN)).ToList();
                        int STT = listGanMay.Count() + 1;
                        int MaxViTri = ganvt.Count();
                        int STTVTGan = STT % MaxViTri == 0 ? MaxViTri : STT % MaxViTri;
                        long? STTDia = 1;
                        if (STT % MaxViTri == 0)
                        {
                            STTDia = STT / MaxViTri;
                        ...
                        {
                            STTDia = STT / MaxViTri + 1;
                        }
                        var VitriGan = ganvt.FirstOrDefault(x => x.IDMayXN.Equals(pmMay.IDMayXN) && x.STT == STTVTGan);
                        if (VitriGan.isTest != true)
                        {
                            PSCM_GanViTriCT ganvtCT = new PSCM_GanViTriCT();
                            ganvtCT.STTVT = STTVTGan;
                            ganvtCT.STTDia = STTDia;
                            ganvtCT.ViTri = STTDia + VitriGan.TenViTri;
                            ganvtCT.isTest = false;
                            ganvtCT.STT = STT;
                            ganvtCT.IDMayXN = pmMay.IDMayXN;
                            ganvtCT.IDLanDucLo = IDLanDucLo;
                            ganvtCT.isDaDuyet = false;
                            ganvtCT.isXoa = false;
                ...
                #endregion
                var lganvt = db.PSCM_GanViTris.Where(x => x.IDMayDucLo.Equals(MayDucLo) && x.isDaDuyet != true && x.isXoa != true && x.IDLanDucLo.Equals(IDLanDucLo)).ToList();
                GanVTNew.STT_bang = lganvt.Count + 1;
                GanVTNew.IDRowLanDucLo = dSGanMay.IDRowLanDucLo;
                db.PSCM_GanViTris.InsertOnSubmit(GanVTNew);
                db.SubmitChanges();
                var GanVTNewAdd = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(IDLanDucLo) && x.MaGoiXN.Equals(pm.MaGoiXN) && x.STT_bang == GanVTNew.STT_bang );
                if (GanVTNewAdd != null)
                {
                   db.PSCM_GanViTriCTs.Where(x => x.IDLanDucLo.Equals(IDLanDucLo) && x.IDRowGanXN == null).ToList().ForEach(y=>y.IDRowGanXN= GanVTNewAdd.IDRowGanXN);
                   db.SubmitChanges();
                }
                gvc.Result = true;
            }
        ...
        public PSCM_GanViTri GetGanViTri(bool isTest,string MayDucLo,string IDLanDucLo,long IDRowLanDucLo,string GiaTriTest,int STTBang, PSCMGanViTriChung pm)
        {
            PSCM_GanViTri gvv = new PSCM_GanViTri();
            try
            {
                PSCM_GanViTri gvt = db.PSCM_GanViTris.FirstOrDefault(x => x.IDMayDucLo.Equals(MayDucLo) && x.isDaDuyet != true && x.isXoa != true && x.IDLanDucLo.Equals(IDLanDucLo) && x.STT_bang.Equals(STTBang));
                if(gvt!=null)
                {
                    gvv=gvt;
                }
                else
                {
                    PSCM_GanViTri GanVTNew = new PSCM_GanViTri();
                    GanVTNew.IDLanDucLo = IDLanDucLo;
                    GanVTNew.IDRowLanDucLo = IDRowLanDucLo;
                    GanVTNew.IDMayDucLo = MayDucLo;
                    GanVTNew.isDaDuyet = false;
                    GanVTNew.isXoa = false;
                    GanVTNew.STT_bang = STTBang;
                    GanVTNew.MaGoiXN = pm.MaGoiXN;
                    GanVTNew.MaXetNghiem = pm.MaXetNghiem;
                    GanVTNew.MaPhieu = pm.MaPhieu;
                    db.PSCM_GanViTris.InsertOnSubmit(GanVTNew);
                    db.SubmitChanges();
                    PSCM_GanViTri gv = db.PSCM_GanViTris.FirstOrDefault(x => x.IDMayDucLo.Equals(MayDucLo) && x.isDaDuyet != true && x.isXoa != true && x.IDLanDucLo.Equals(IDLanDucLo) && x.STT_bang.Equals(STTBang));
                    gvv=gv;
                }
        ...
        public bool GanVTTest(string IDMayXN, string IDLanDucLo, string IDMayDucLo,long IDRowLanDucLo)
        {
            bool result = false;
            try
            {
                var IDRowLanDL = db.PSCM_DSGanMays.FirstOrDefault(x => x.IDMayDucLo.Equals(IDMayDucLo) && x.isDaDuyet != true && x.isDaXoa != true && x.IDLanDucLo.Equals(IDLanDucLo)).IDRowLanDucLo;
                var lganvt = db.PSCM_GanViTris.Where(x => x.IDMayDucLo.Equals(IDMayDucLo) && x.isDaDuyet != true && x.isXoa != true && x.IDLanDucLo.Equals(IDLanDucLo)).ToList();
                int STTBang = lganvt.Count + 1;
                var ganvt = db.PSMapsViTriMayXNs.Where(x => x.IDMayXN.Equals(IDMayXN)).ToList();
                var listGanMay = db.PSCM_GanViTriCTs.Where(x => x.isDaDuyet != true && x.isXoa != true && x.IDLanDucLo.Equals(IDLanDucLo) && x.IDMayXN.Equals(IDMayXN)).ToList();
                int STT = listGanMay.Count() + 1;
                int MaxViTri = ganvt.Count();
                int STTVTGan = STT % MaxViTri == 0 ? MaxViTri : STT % MaxViTri;
                long? STTDia = 1;
                ...
                var VitriGan = ganvt.FirstOrDefault(x => x.IDMayXN.Equals(IDMayXN) && x.STT == STTVTGan);
                if (VitriGan.isTest == true)
                {
                    PSCM_GanViTri GanVTNew = new PSCM_GanViTri();
                    GanVTNew.STT_bang = STTBang;
                    GanVTNew.MaGoiXN = "DVTest";
                    GanVTNew.IDLanDucLo = IDLanDucLo;
                    GanVTNew.isDaDuyet = false;
                    GanVTNew.isXoa = false;
                    GanVTNew.IDMayDucLo = IDMayDucLo;
                    GanVTNew.IDRowLanDucLo = IDRowLanDucLo;
                    GanVTNew.MaXetNghiem = VitriGan.GiaTriTest;
                    db.PSCM_GanViTris.InsertOnSubmit(GanVTNew);
                    db.SubmitChanges();
                    var GanVTNewAdd = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(IDLanDucLo) && x.MaGoiXN.Equals("DVTest") && x.STT_bang.Equals(STTBang));
                    if (GanVTNew != null)
                    {
                        PSCM_GanViTriCT ganvtCT = new PSCM_GanViTriCT();
                        ganvtCT.STTVT = STTVTGan;
                        ...
                        ganvtCT.isTest = true;
                        ganvtCT.STT = STT;
                        ganvtCT.IDMayXN = IDMayXN;
                        ganvtCT.IDLanDucLo = IDLanDucLo;
                        ganvtCT.IDRowGanXN = GanVTNew.IDRowGanXN;
                        ganvtCT.isDaDuyet = false;
                        ganvtCT.isXoa = false;
                        db.PSCM_GanViTriCTs.InsertOnSubmit(ganvtCT);
                        db.SubmitChanges();
        ...
        public PsReponse DoiViTriGanXN(PSCMGanViTriChung pm1, PSCMGanViTriChung pm2, string IDMayXN)
        {
            PsReponse reponse = new PsReponse();
            try
            {
                var vt1 = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(pm1.IDLanDucLo) && x.IDRowGanXN == pm1.IDRowGanXN);
                var vt2 = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(pm2.IDLanDucLo) && x.IDRowGanXN == pm2.IDRowGanXN);
                int Max = db.PSCM_GanViTriCTs.Where(x => x.IDMayXN.Equals(IDMayXN) && x.isDaDuyet != true).ToList().Count();
                foreach (var vt in vt1.PSCM_GanViTriCTs)
                {
                    if (vt.IDMayXN.Equals(IDMayXN))
                    {
                        foreach (var vtnew in vt2.PSCM_GanViTriCTs)
                        {
                            if (vtnew.IDMayXN.Equals(IDMayXN))
                            {
                                if (vtnew.STT == Max && vt.isTest == true)
                                {
                                    vt.isTest = false;
                                    vt.IDRowGanXN = vtnew.IDRowGanXN;
                                    db.SubmitChanges();
                                    long? VT = vt1.STT_bang;
                                    db.PSCM_GanViTris.DeleteOnSubmit(vt1);
                                    db.SubmitChanges();
                                    db.PSCM_GanViTriCTs.DeleteOnSubmit(vtnew);
                                    var ganvt = db.PSCM_GanViTris.Where(x => x.STT_bang >= VT).ToList();
                                ...
                                {
                                    long? temp = vtnew.IDRowGanXN;
                                    bool? tempTet = vtnew.isTest;
                                    vtnew.isTest = vt.isTest;
                                    vtnew.IDRowGanXN = vt.IDRowGanXN;
                                    vt.isTest = tempTet;
                                    vt.IDRowGanXN = temp;
                                    db.SubmitChanges();
                                    reponse.Result = true;
        ...
        public PsReponse ThemGhiChuGanVT(PSCMGanViTriChung pm)
        {
            ...
            try
            {
                PSCM_GanViTri viTri = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(pm.GGanViTri.IDLanDucLo) && x.IDRowGanXN == pm.GGanViTri.IDRowGanXN
                && x.isDaDuyet != true);
                if (viTri != null)
                {
                    ...
                    if (viTri.PSCM_GanViTriCTs.Count() > 0)
                    {
                        foreach (var vtc in viTri.PSCM_GanViTriCTs)
                        {
                            switch (vtc.IDMayXN)
                            {
                                case "MAYXN001":
                                    {
                                        vtc.GhiChuCT = pm.MAYXN001.GhiChuCT;
                                        vtc.isXoa = false;
                                        db.SubmitChanges();
                                        break;
                                    }
                                case "MAYXN002":
                                    {
                                        vtc.GhiChuCT = pm.MAYXN002.GhiChuCT;
                                        vtc.isXoa = false;
                                        db.SubmitChanges();
                                        break;
                                    }
        ...
        public PsReponse GetMaPhieuTrongDSGanVT(string MaPhieu, string MayDucLo)
        {
            PsReponse reponse = new PsReponse();
            try
            {
                db.PSCM_DSGanMays.FirstOrDefault(x => x.IDMayDucLo.Equals(MayDucLo) && x.isDaDuyet != true && x.isDaXoa != true);
            }
            catch
            {
        ...
        public PsReponse DeleteDanhSachGanXNLuu(PSCMGanViTriChung pm)
        {
            ...
            try
            {
                PSCM_GanViTri viTri = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(pm.GGanViTri.IDLanDucLo) && x.IDRowGanXN == pm.GGanViTri.IDRowGanXN
                && x.isDaDuyet != true);
                var ds = db.PSCM_GanViTris.Where(x => x.isDaDuyet != true && x.IDLanDucLo.Equals(pm.GGanViTri.IDLanDucLo) && x.isXoa != true).ToList();
        ...
        public PsReponse DeleteDanhSachGanXNLuuMax(PSCMGanViTriChung pm)
        {
            PsReponse reponse = new PsReponse();
            try
            {
                PSCM_GanViTri viTri = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(pm.IDLanDucLo) && x.IDRowGanXN == pm.IDRowGanXN && x.isDaDuyet != true);
                if(viTri!=null)
                {
                    foreach(var vt in viTri.PSCM_GanViTriCTs)
                    {
                        db.PSCM_GanViTriCTs.DeleteOnSubmit(vt);
                    }
                    db.SubmitChanges();
                    db.PSCM_GanViTris.DeleteOnSubmit(viTri);
                }
        ...
        public PsReponse DeleteCTGanXNLuu(PSCM_GanViTriCT gan)
        {
            ...
            try
            {
                var kq = db.PSCM_GanViTriCTs.FirstOrDefault(x => x.IDLanDucLo == gan.IDLanDucLo && x.IDMayXN == gan.IDMayXN && x.STTDia == gan.STTDia && x.STTVT == gan.STTVT);
                if (kq != null)
                {
        ...
        public PsReponse DeleteDanhSachAllGanXNLuu(string IDLanDucLo, string IDNhanVien)
        {
            PsReponse gvc = new PsReponse();
            try
            {
                PSCM_DSGanMay dsmay = db.PSCM_DSGanMays.FirstOrDefault(x => x.IDLanDucLo.Equals(IDLanDucLo) && x.isDaDuyet != true && x.isDaXoa != true);
                if (dsmay != null)
                {
                    dsmay.isDaXoa = true;
                    dsmay.DateDel = DateTime.Now;
                    dsmay.IDNVDel = IDNhanVien;
                    db.SubmitChanges();
                }
        ...
        public PsReponse DeleteDanhSachAllGanXNLuuNew(List<PSCMGanViTriChung> pm)
        {
            PsReponse reponse = new PsReponse();
            try
            {
                ...
                {
                    PSCM_GanViTri viTri = db.PSCM_GanViTris.FirstOrDefault(x => x.IDLanDucLo.Equals(gc.IDLanDucLo) && x.IDRowGanXN == gc.IDRowGanXN && x.isDaDuyet != true && x.isXoa != true);
                    if (viTri != null)
                    {
                        if (viTri.PSCM_GanViTriCTs.Count() > 0)
                        {
        ...
        public PsReponse DuyetCapMaGanMayXN(string IDLanDucLo, string IDNhanVien)
        {
            PsReponse reponse = new PsReponse();
            try
            {
                ...
                if (dsmay != null)
                {
                    dsmay.isDaDuyet = true;
                    dsmay.DateEnd = DateTime.Now;
        ...
        public List<PSCMGanViTriChung> GetDanhSachGanXNLuuNew(string MayDucLo,string IDLanDucLo)
        {
            List<PSCMGanViTriChung> gvc = new List<PSCMGanViTriChung>();
            try
            {
                List<PSCM_GanViTri> lst = db.PSCM_GanViTris.Where(x => x.isDaDuyet != true && x.IDMayDucLo.Equals(MayDucLo) && x.isXoa != true && x.IDLanDucLo.Equals(IDLanDucLo)).OrderBy(x => x.STT_bang).ToList();
                foreach (var ls in lst)
                {
                    PSCMGanViTriChung gv = new PSCMGanViTriChung();
                    gv.GGanViTri = new PSCM_GanViTri();
                    gv.GGanViTri = ls;
                    if (ls.PSCM_GanViTriCTs.Count() > 0)
                    {
                        gv.may = new List<PSDanhMucMayXN>();
                        var dsmay = GetDSMayXNTheoMayDucLo(MayDucLo);
                        foreach (var ctg in ls.PSCM_GanViTriCTs)
                        {
                            if (ctg.isXoa != true)
                            {
                                switch (ctg.IDMayXN)
                                {
                                    case "MAYXN001":
                                        {
                                            gv.CTMAYXN001 = new PSCM_GanViTriCT();
                                            gv.CTMAYXN001 = ctg;
                                            ...
                                            mayxn.isSuDung = true;
                                            var ten = dsmay.FirstOrDefault(x => x.IDMayXN.Equals(ctg.IDMayXN));
                                            mayxn.TenMayXN = ten != null ? ten.TenMayXN : "Máy 3 bệnh";
                                            gv.may.Add(mayxn);
                                            break;
                                        }
                                    case "MAYXN002":
                                        {
                                            gv.CTMAYXN002 = new PSCM_GanViTriCT();
                                            gv.CTMAYXN002 = ctg;
                                            ViTriXN vt = new ViTriXN();
                                            vt.GhiChuCT = ctg.GhiChuCT;
                                            vt.isTest = ctg.isTest;
                                            ...
                                            mayxn.isSuDung = true;
                                            var ten = dsmay.FirstOrDefault(x => x.IDMayXN.Equals(ctg.IDMayXN));
                                            mayxn.TenMayXN = ten != null ? ten.TenMayXN : "Máy 2 bệnh";
                                            gv.may.Add(mayxn);
                                            break;
                                        }
                                }
        ...
        public PsReponse SaveDSMapViTri(PSCMGanViTriChung gvt)
        {
            ...
            try
            {
                ...
                && x.MaXetNghiem == gvt.MaXetNghiem && x.IDLanDucLo == gvt.IDLanDucLo && x.STT_bang == gvt.STT_bang);
            }
            catch
            {
        ...
        public string GetTenDanToc(int maDanToc)
        {
            try
            {
               return db.PSDanhMucDanTocs.FirstOrDefault(p => p.IDDanToc==maDanToc).TenDanToc;
            }
            catch { return null; }
        }
        ...
        public List<PSChiTietDanhGiaChatLuong> GetChiTietDanhGiaMạuKhongDatTrenPhieu(string maPhieu, string maTiepNhan)
        {
            ...
            lst = db.PSChiTietDanhGiaChatLuongs.Where(p => p.MaTiepNhan == p.MaTiepNhan && p.IDPhieu == maPhieu && p.isXoa !=true).ToList();
            return lst;
        }
        ...
        public PsReponse LuuKetQuaXN(KetQua_XetNghiem KQ)
        {
            ...
            try
            {
                ...
                if (KetQua != null)
                {
                    foreach (var item in KetQua.PSXN_KetQua_ChiTiets)
                    {
                        var KetQuaCT = KQ.KetQuaChiTiet.FirstOrDefault(x => x.MaKQ == item.MaKQ && x.MaThongSo == item.MaThongSoXN && x.MaXN == item.MaXetNghiem);
                        if (KetQuaCT != null)
                        {
                    ...
                    KetQua.isXoa = false;
                    if (KetQua.MaGoiXN.Equals("DVGXNL2"))
                    {
                        isXNL2 = true;
                    }
                    else
                ...
                if (isCoKQ)
                {
                    ...
                    if (TraKetQua == null)
                    {
                        ...
                        var phieu = db.PSPhieuSangLocs.FirstOrDefault(p => p.isXoa != true && p.IDPhieu == KQ.maPhieu && p.IDCoSo == KQ.maDonVi);
                        if (phieu.isLayMauLan2 == true)
                        {
        ...
        #endregion
        public string GetVietTatDV(string MaDV)
        {
            string viettat = string.Empty;
            try
            {
                viettat = db.PSDanhMucDonViCoSos.FirstOrDefault(x => x.MaDVCS.Equals(MaDV)).VietTatDV;
            }
            catch
            {
                viettat = string.Empty;
            }
            return viettat;
        }
        ...
        public List<PsDanhSachMauDuongTinh> GetDanhSachDuongTinh(DateTime NgayBD, DateTime NgayKT, string TenDichVu, string DonVi, string Min, string Max)
        {
            ...
            try
            {
                    ...
                    try
                    {
                        ...
                        foreach (var ls in lstphieu)
                        {
                            if (ls.isLayMauLan2 == true)
                            {
                                ...
                                var kq2 = db.PSXN_TraKetQuas.FirstOrDefault(x => x.MaPhieu.Equals(duongtinh.MaPhieuL2) && x.isXoa != true);
                                ...
                                var phieu1 = db.PSXN_TraKQ_ChiTiets.FirstOrDefault(x => x.MaPhieu.Equals(duongtinh.MaPhieuL1) && x.isXoa != true && x.MaDichVu.Equals(TenDichVu));
                                if (phieu1 != null)
                                {
                            ...
                            else
                            {
                                ...
                                var kq1 = db.PSXN_TraKetQuas.FirstOrDefault(x => x.MaPhieu.Equals(duongtinh.MaPhieuL1) && x.isXoa != true);
                                var ctkq = kq1.PSXN_TraKQ_ChiTiets.FirstOrDefault(x => x.MaDichVu.Equals(TenDichVu) && x.isXoa != true);
                                duongtinh.KetQuaCuoiL1 = ctkq.GiaTriCuoi;
                                duongtinh.KetQua1L1 = ctkq.GiaTri1;
                                ...
                                duongtinh.TenDichVu = ctkq.TenThongSo;
                                duongtinh.MaDichVu = ctkq.MaDichVu;
                                if(string.IsNullOrEmpty(duongtinh.MaPhieuL2))
                                {
                                    duongtinh.NgayLayMau = ls.NgayGioLayMau;
                                    duongtinh.NgayNhanMau = ls.NgayNhanMau;
                                    duongtinh.MaGoiXN = ls.MaGoiXN;
                ...
                dsduongtinh = dsduongtinh.OrderBy(x => x.is2L2).ThenBy(x => x.is2L1).ThenBy(x => x.is1L2).ThenBy(x => x.MaGoiXN).ThenBy(x => x.VietTatDV).ToList();
                int stt = 1;
                    ...
                    try
                    {
                        ...
                        foreach (var ls in lstphieu)
                        {
                            ...
                            else
                            {
                                ...
                                var kq1=db.PSXN_TraKQ_ChiTiets.FirstOrDefault(x => x.MaPhieu.Equals(duongtinh.MaPhieuL1) && x.isXoa != true && x.MaDichVu.Equals(duongtinh.MaDichVu));
                                duongtinh.TenDichVu = kq1.TenThongSo;
                                duongtinh.KetQua1L1 = kq1.GiaTri1;
                                duongtinh.KetQua1L2 = kq1.GiaTri2;
        ...
        #endregion
        #region Báo cáo tùy chọn theo dịch vụ
            ...
            try
            {
                ...
                if (MaDV.Equals("all"))
                {
                    var res = (from ph in db.PSPhieuSangLocs
                               ...
                               join ct in db.PSXN_TraKQ_ChiTiets on kq.MaPhieu equals ct.MaPhieu
                               where kq.NgayCoKQ.Value.Date >= NgayBD.Date && kq.NgayCoKQ.Value.Date <= NgayKT.Date
                               && kq.isXoa != true && ph.isXoa != true && kq.isTraKQ == true
                               select new { ph.MaBenhNhan }).Distinct().ToList();
            ...
            public List<PSThongKeTheoDonVi> LoadDSThongKeDonViCT(List<PSBaoCaoTuyChonDonVi> lst)
        {
            ...
            try
            {
                var lstdv = lst.Select(x => x.phieu1.IDCoSo).Distinct().ToList();
                int STT = 1;
        ...
        public List<PSThongKeTheoDonVi> LoadDSThongKe(List<PSBaoCaoTuyChonDonVi> lst, List<PSThongKeTheoDonVi> lstdv)
        {
            List<PSThongKeTheoDonVi> lstresult = new List<PSThongKeTheoDonVi>();
            try
            {
                foreach (PSThongKeTheoDonVi dv in lstdv)
                {
                    try
                    {
                        ...
                        if (kq.Count() > 0)
                        {
                            lstresult.Add(LoadDSThongKeDV(kq,dv,1));
                        }
                    }
                    catch
                    {
        ...
        public PSThongKeTheoDonVi LoadDSThongKeDV(List<PSBaoCaoTuyChonDonVi> lst, PSThongKeTheoDonVi dv, int phieu)
        {
            ...
            List<PSPhieuSangLoc> ph1 = new List<PSPhieuSangLoc>();
            try
            {
                if (phieu == 1)
                {
                    kq = lst.Where(y => y.phieu1.IDCoSo.Equals(dv.MaDV)).ToList();
                    ph1 = kq.Select(x => x.phieu1).ToList();
                }
                else
                {
                    kq= lst.Where(y => y.phieu2.IDCoSo.Equals(dv.MaDV)).ToList();
                    ph1 = kq.Select(x => x.phieu2).ToList();
                }
                if (kq.Count() > 0)
                {
